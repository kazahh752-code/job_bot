...
# Вакансия ПОДХОДИТ если название содержит хотя бы одно из этих слов
INCLUDE_KEYWORDS = [
    "личный водитель", "персональный водитель", "семейный водитель",
    "водитель-помощник", "водитель помощник", "водитель руководителя",
    "водитель директора", "водитель вип", "водитель vip",
    "частный водитель", "водитель для семьи", "водитель на представительский",
    "personal driver", "водитель-ассистент", "водитель ассистент",
]

# Вакансия ОТСЕИВАЕТСЯ если название содержит хотя бы одно из этих слов
EXCLUDE_KEYWORDS = [
    "курьер", "газель", "фура", "грузовик", "межгород", "дальнобой",
    "категория е", "кат е", "экспедитор", "логист", "доставка",
    "пятёрочка", "пятерочка", "магнит", "wildberries", "ozon", "озон",
    "самокат", "яндекс", "такси", "агрегатор", "маршрутка",
    "автобус", "троллейбус", "трамвай", "машинист", "спецтехника",
    "автобетон", "кран", "экскаватор", "погрузчик", "дальние рейсы",
    "категория c", "категория с", "кат c", "кат с",
]
...
async def check_subscription(bot: Bot, db: Database, sub: dict):
    ...
    # Фильтруем — оставляем только релевантные вакансии
    all_jobs = [j for j in all_jobs if is_relevant_job(j)]
